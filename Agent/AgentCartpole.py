...
import logging  
import math
from NetworkArchitecture.NeuralNetworkCartpole import NeuralNetwork
...
class Agent:
    def __init__(self, n_observe, n_actions): # initialize Agent
    ...
    def create_network(self):
        return NeuralNetwork(n_observe=self.n_observe,
    ...
    def select_action(self, state, epsilon=-1):
        ...
        value, index = torch.max(predicted, 0)
        print(value, index)
        return index.item()
        # s = nn.Softmax(dim=0)
        # probs = s(predicted)
    ...
    def compute_loss(self, states, actions, rewards, next_states, dones, gamma=GAMMA):
        ...
        for param in self.policy_network.parameters():
            print("in layer", cnt, torch.mean(param.grad))
            grads.append(param.grad.view(-1))
            cnt += 1
        grads = torch.cat(grads)
        print("mean gradient is", torch.mean(grads))
